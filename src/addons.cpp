...
typedef std::unique_ptr<llama_sampling_context, decltype(&llama_sampling_free)> LlamaCppSampling;
...
public:
  // construct({ model, embedding, n_ctx, n_batch, n_threads, n_gpu_layers, use_mlock, use_mmap }): LlamaContext throws error
  ...
  static void Export(Napi::Env env, Napi::Object &exports) {
    Napi::Function func = DefineClass(env, "LlamaContext", {
      InstanceMethod<&LlamaContext::GetSystemInfo>("getSystemInfo", static_cast<napi_property_attributes>(napi_enumerable)),
      InstanceMethod<&LlamaContext::Completion>("completion", static_cast<napi_property_attributes>(napi_enumerable)),
      InstanceMethod<&LlamaContext::StopCompletion>("stopCompletion", static_cast<napi_property_attributes>(napi_enumerable)),
      InstanceMethod<&LlamaContext::SaveSession>("saveSession", static_cast<napi_property_attributes>(napi_enumerable)),
      InstanceMethod<&LlamaContext::LoadSession>("loadSession", static_cast<napi_property_attributes>(napi_enumerable))
    });
...
class LlamaCompletionWorker : public Napi::AsyncWorker, public Napi::Promise::Deferred {
  LlamaContext* _ctx;
  gpt_params _params;
...
public:
  LlamaCompletionWorker(const Napi::CallbackInfo& info, LlamaContext* ctx, Napi::Function callback, gpt_params params, std::vector<std::string> stop_words = {}) :
...
protected:
  ...
  void Execute() {
    ...
    LlamaCppSampling sampling { llama_sampling_init(_params.sparams), llama_sampling_free };

    std::vector<llama_token> prompt_tokens = ::llama_tokenize(ctx, _params.prompt, add_bos);
    ...
    for (int i = 0; i < max_len || _stop; i++) {
      ...
      int ret = llama_decode(ctx, llama_batch_get_one(embd->data() + n_cur, n_input, n_cur, 0));
      if (ret < 0) {
        SetError("Failed to decode token, code: " + std::to_string(ret));
        break;
      }
      // sample the next token
      const llama_token new_token_id = llama_sampling_sample(sampling.get(), ctx, nullptr);
      ...
      if (_has_callback) {
        ...
        _tsfn.BlockingCall(c_token, [](Napi::Env env, Napi::Function jsCallback, const char *value) {
      ...
      if (!_stop_words.empty()) {
        const size_t stop_pos = findStoppingStrings(generated_text, token.size());
  ...
  void OnOK() {
    ...
    result.Set("tokens_evaluated", Napi::Number::New(Napi::AsyncWorker::Env(), count));
    result.Set("generated_text", Napi::String::New(Napi::AsyncWorker::Env(), generated_text));
...
class SaveSessionWorker : public Napi::AsyncWorker, public Napi::Promise::Deferred {
  std::string _path;
  LlamaContext* _ctx;
...
protected:
  void Execute() {
    ...
    if (!llama_state_save_file(_ctx->getContext(), _path.c_str(), _ctx->getTokens()->data(), _ctx->getTokens()->size())) {
      SetError("Failed to save session");
    }
    _ctx->getMutex().unlock();
  }
  ...
  void OnOK() {
    Resolve(AsyncWorker::Env().Undefined());
  }
...
class LoadSessionWorker : public Napi::AsyncWorker, public Napi::Promise::Deferred {
  std::string _path;
  LlamaContext* _ctx;
  size_t count = 0;
...
protected:
  void Execute() {
    ...
    if (!llama_state_load_file(_ctx->getContext(), _path.c_str(), _ctx->getTokens()->data(), _ctx->getTokens()->capacity(), &count)) {
...
Napi::Value LlamaContext::Completion(const Napi::CallbackInfo& info) {
  ...
  params.n_predict = options.Has("n_predict") ? options.Get("n_predict").ToNumber() : -1;
  params.sparams.temp = options.Has("temperature") ? options.Get("temperature").ToNumber() : 0.80f;
  params.sparams.top_k = options.Has("top_k") ? options.Get("top_k").ToNumber() : 40;
  params.sparams.top_p = options.Has("top_p") ? options.Get("top_p").ToNumber() : 0.95f;
  params.sparams.min_p = options.Has("min_p") ? options.Get("min_p").ToNumber() : 0.05f;
  params.sparams.tfs_z = options.Has("tfs_z") ? options.Get("tfs_z").ToNumber() : 1.00f;
  params.sparams.mirostat = options.Has("mirostat") ? options.Get("mirostat").ToNumber() : 0;
  params.sparams.mirostat_tau = options.Has("mirostat_tau") ? options.Get("mirostat_tau").ToNumber() : 5.00f;
  params.sparams.mirostat_eta = options.Has("mirostat_eta") ? options.Get("mirostat_eta").ToNumber() : 0.10f;
  params.sparams.penalty_last_n = options.Has("penalty_last_n") ? options.Get("penalty_last_n").ToNumber() : 64;
  params.sparams.penalty_repeat = options.Has("penalty_repeat") ? options.Get("penalty_repeat").ToNumber() : 1.00f;
  params.sparams.penalty_freq = options.Has("penalty_freq") ? options.Get("penalty_freq").ToNumber() : 0.00f;
  params.sparams.penalty_present = options.Has("penalty_present") ? options.Get("penalty_present").ToNumber() : 0.00f;
  params.sparams.penalize_nl = options.Has("penalize_nl") ? options.Get("penalize_nl").ToBoolean() : false;
  params.sparams.typical_p = options.Has("typical_p") ? options.Get("typical_p").ToNumber() : 1.00f;
  params.ignore_eos = options.Has("ignore_eos") ? options.Get("ignore_eos").ToBoolean() : false;
  params.sparams.grammar = options.Has("grammar") ? options.Get("grammar").ToString().Utf8Value() : "";
  params.n_keep = options.Has("n_keep") ? options.Get("n_keep").ToNumber() : 0;
  params.seed = options.Has("seed") ? options.Get("seed").ToNumber() : LLAMA_DEFAULT_SEED;
  ...
  auto worker = new LlamaCompletionWorker(info, this, callback, params, stop_words);
  worker->Queue();
  compl_worker = worker;

...
DecodeAudioTokenWorker::DecodeAudioTokenWorker(const Napi::CallbackInfo &info, llama_model *model, llama_context *ctx,
                  int n_threads, const std::vector<llama_token> &tokens)
  : AsyncWorker(info.Env()), Deferred(info.Env()), _model(model), _ctx(ctx), _n_threads(n_threads), _tokens(tokens) {
}
